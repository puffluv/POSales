...
namespace POSales
{
    public partial class ProductModule : Form
    {
        ...
        public void LoadCategory()
        {
            ...
            cboCategory.DataSource = null; // Очищаем DataSource для корректного обновления.
            DataTable categoryTable = dbcon.getTable("SELECT * FROM tbCategory");
            cboCategory.DataSource = categoryTable;
            cboCategory.DisplayMember = "category";
            cboCategory.ValueMember = "id";

            // Настройка для автозаполнения
            ...
            // Добавление данных для автозаполнения
            AutoCompleteStringCollection autoCompleteData = new AutoCompleteStringCollection();
            foreach (DataRow row in categoryTable.Rows)
            {
                autoCompleteData.Add(row["category"].ToString());
            }
            cboCategory.AutoCompleteCustomSource = autoCompleteData;
        }

        public void LoadBrand()
        {
            cboBrand.Items.Clear();
            cboBrand.DataSource = null; // Очищаем DataSource для корректного обновления.
            DataTable brandTable = dbcon.getTable("SELECT * FROM tbBrand");
            cboBrand.DataSource = brandTable;
            cboBrand.DisplayMember = "brand";
            cboBrand.ValueMember = "id";

            // Настройка для автозаполнения
            cboBrand.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cboBrand.AutoCompleteSource = AutoCompleteSource.CustomSource;

            // Добавление данных для автозаполнения
            AutoCompleteStringCollection autoCompleteData = new AutoCompleteStringCollection();
            foreach (DataRow row in brandTable.Rows)
            {
                autoCompleteData.Add(row["brand"].ToString());
            }
            cboBrand.AutoCompleteCustomSource = autoCompleteData;
        }
